...
async function geo() {
  ...
  if (process.env.GEO_DOWN === "primary")
    return { provider: "ipapi.co", country: "Fallback", city: "Development" };
...
async function submit(body, req) {
  ...
  if (
    ...
    return { httpStatus: 400, error: "widgetId and data are required" };
  const w = state.widgets[body.widgetId];
  if (!w) return { httpStatus: 404, error: "Widget not found" };
  if (body.data.website) return { httpStatus: 400, error: "Spam detected" };
  const email = body.data.email;
  if (typeof email !== "string" || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email))
    return { httpStatus: 400, error: "Valid email is required" };
  const ip = req.socket.remoteAddress || "unknown";
  if (!allowedSubmission(ip, w.id))
    return { httpStatus: 429, error: "Rate limit exceeded" };
  const key = req.headers["idempotency-key"];
  if (key && state.idempotency[`${w.id}:${key}`])
    return {
      httpStatus: 200,
      ...state.idempotency[`${w.id}:${key}`],
      idempotent: true,
    };
  const s = {
    id: crypto.randomUUID(),
  ...
  const result = { id: s.id, status: "accepted", geo: s.geo };
  if (key) state.idempotency[`${w.id}:${key}`] = result;
  persist();
  if (postgres.enabled())
    postgres
      .insertSubmission(s)
      .catch((error) =>
        console.error("postgres persistence failed", error.message),
      );
  setImmediate(() => {
    if (process.env.SIDE_EFFECT_DOWN === "1")
      console.error("notification failed", s.id);
    else console.log("notification", s.id);
  });
